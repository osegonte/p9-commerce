        ...
        <div className="px-4 sm:px-6 lg:px-8 flex items-center justify-between h-14 sm:h-16">
          ...
          >
            <div className="flex flex-col justify-center gap-[5px] w-5 h-5">
              <span className={`block h-[1.5px] w-full transition-all duration-300 origin-center ${isDark ? "bg-black" : "bg-white"} ${menuOpen ? "rotate-45 translate-y-[6.5px]" : ""}`} />
              <span className={`block h-[1.5px] w-full transition-all duration-300 ${isDark ? "bg-black" : "bg-white"} ${menuOpen ? "opacity-0" : "opacity-100"}`} />
              <span className={`block h-[1.5px] w-full transition-all duration-300 origin-center ${isDark ? "bg-black" : "bg-white"} ${menuOpen ? "-rotate-45 -translate-y-[6.5px]" : ""}`} />
            </div>
            <span className={`text-[11px] tracking-[0.15em] uppercase font-light hidden sm:inline transition-colors duration-500 ${isDark ? "text-black" : "text-white"}`}>
              menu
            </span>
          </button>
          ...
          <div className="flex items-center gap-5 sm:gap-6">
            ...
            <Link href="/cart" className={`transition-colors duration-500 relative ${iconColor}`} aria-label="Cart">
              ...
              {mounted && totalItems > 0 && (
                <span className="absolute -top-1.5 -right-1.5 min-w-[16px] h-4 bg-[#1a1a1a] text-white text-[9px] font-medium rounded-full flex items-center justify-center px-[3px] leading-none border border-white">
